package me.portafolio.inclusiveTrip.dao;
...
import me.portafolio.inclusiveTrip.dao.rowmappers.*;
...
public class ImplUsuarioDao implements CRUD<Usuario> {
	...
	final String GETADMINS = "SELECT u.idusuario, u.username, u.nombre, u.apellido, u.fechanacimiento, u.clave, u.run, a.idadministrativo, a.area "
			+ "FROM usuario u " + "INNER JOIN administrativo a " + "ON u.run = a.rutadmin;";
	final String GETCLIENTS = "SELECT u.idusuario, u.username, u.nombre, u.apellido, u.fechanacimiento, u.clave, u.run, c.idcliente, c.telefono, c.email "
			+ "FROM usuario u " + "INNER JOIN cliente c " + "ON u.run = c.rutcliente " + "ORDER BY u.idusuario ";
	...
	/**
	 * @category Metodo
	 * @param getUserPass() Este metodo retorna un usuario, si esque la contraseña y
	 *                      el username son correctos
	 */
	@Override
	...
	/**
	 * @category Metodo
	 * @param mostrarUsuarios() Este metodo retorna una lista con todos los usuarios
	 *                          y sus datos especificos correspondientes, ya sea
	 *                          para administrativo o cliente
	 */
	@Override
